...
import { GetHtml } from '../webviews/models';

const { joinPath } = vscode.Uri;

export const getHtml = <T>(options: GetHtml<T>, nonce: string): string => {
  ...
  const styleUri = webview.asWebviewUri(
    vscode.Uri.joinPath(extensionPath, 'resources', 'css', 'styles.css')
  );
